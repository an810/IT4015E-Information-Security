...
def run(dir, data, pwd, cipher_file_path=None):
    ...
    if dir == Direction.ENCRYPT:
        with open(cipher_file_path, "wb") as cipher_file:
            for index in range(0, len(data), part_size):
                ...
                encrypted_part = encrypt(part, pwd)
                cipher_file.write(bytes(encrypted_part))
                result += encrypted_part
    elif dir == Direction.DECRYPT:
        print(cipher_file_path)
        with open(cipher_file_path, "rb") as cipher_file:
            cipher_data = cipher_file.read()
            for index in range(0, len(cipher_data), part_size):
                part = cipher_data[index:index + part_size]
